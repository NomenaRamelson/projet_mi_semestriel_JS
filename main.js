...
function loadRecentTransactions(monthTxs) {
    ...
    if (!recent.length) { 
        txList.innerHTML = '<li class="tx-empty">Aucune transaction ce mois</li>'; 
        return; 
    }
...
function loadGoalsDash(goals) {
    ...
    if (!goals.length) { 
        el.innerHTML = '<div class="tx-empty">Aucun objectif</div>'; 
        return; 
    }
...
function loadUpcomingDash() {
    ...
    if (!list.length) { 
        el.innerHTML = '<li class="tx-empty">Aucune charge à venir</li>'; 
        return; 
    }
    ...
    el.innerHTML = list.map(u => {
        ...
        return `<li class="upcoming-item">
            <div class="upcoming-icon ico--utility">${u.name.charAt(0)}</div>
...
function renderBudget() {
    ...
    document.getElementById('view-budget').innerHTML = `
        <header class="view-header">
            <div>
                <h1 class="view-title">Budget mensuel</h1>
                <p class="view-subtitle">${MONTHS[curMonth]} ${curYear}</p>
            </div>
            <button class="btn-primary" onclick="openAddTransaction()">+ Ajouter</button>
        </header>
        ...
        <div class="dash-body">
                        ...
                        return `<div class="budget-row">
                            <div class="budget-row-top">
                                <span class="budget-cat-name">
                                    <span style="color:${cat.color};font-weight:700">${cat.name.charAt(0)}</span> 
                                    ${escapeHtml(cat.name)}
                                </span>
                                <span class="budget-amounts">
                                    <span style="color:${over?'var(--clr-red)':'var(--clr-text-1)'}">${formatMoney(spent)}</span>
                                    ${budget ? `<span class="goal-target">/ ${formatMoney(budget)}</span>` : ''}
                                </span>
                            </div>
                            ...
                            <div style="display:flex;gap:8px;margin-top:6px;align-items:center">
                                <input type="number" class="budget-input" placeholder="Budget" value="${budget||''}" onchange="setBudget('${bKey}','${cat.id}',this.value)">
            ...
            <div class="dash-right">
                <div class="card">
                    <div class="card-header">
                        <h2 class="card-title">Transactions du mois</h2>
                    </div>
                    <ul class="tx-list">
                        ${[...txs].sort((a,b)=>b.date.localeCompare(a.date)).map(tx => {
                            ...
                            return `<li class="tx-item">
                                <div class="tx-icon ${isI?'tx-icon--income':'tx-icon--expense'}">${cat.name.charAt(0)}</div>
                                <div class="tx-info">
                                    <span class="tx-label">${escapeHtml(tx.label)}</span>
                                    <span class="tx-cat">${dateLabel}</span>
                                </div>
                                <div class="tx-right">
        ...
        <div id="modal-tx" class="modal-overlay" onclick="if(event.target===this)closeModal('modal-tx')">
            <div class="modal-box">
                <div class="modal-header">
                    ...
                    <button class="modal-close" onclick="closeModal('modal-tx')">X</button>
                </div>
                <div class="modal-body">
                    ...
                    <div class="input-group">
                        <label>Libellé</label>
                        <input type="text" id="tx-label" placeholder="Ex: Courses">
                    </div>
                    <div class="input-group">
...
function renderUpcoming() {
    ...
    document.getElementById('view-upcoming').innerHTML = `
        <header class="view-header">
            <div>
                <h1 class="view-title">Dépenses à venir</h1>
                <p class="view-subtitle">Charges récurrentes mensuelles</p>
            </div>
            <button class="btn-primary" onclick="openAddUpcoming()">+ Ajouter</button>
        </header>
                ...
                ${list.map(u => {
                    ...
                    return `<li class="upcoming-item" style="${urgent?'background:var(--clr-amber-light);border-radius:10px':''}">
                        ...
                        <div style="display:flex;align-items:center;gap:12px">
                            <span class="upcoming-amount">- ${formatMoney(u.amount)}</span>
                            <button class="btn-del" onclick="deleteUpcoming('${u.id}')">X</button>
                        </div>
                    </li>`;
        ...
        <div id="modal-upcoming" class="modal-overlay" onclick="if(event.target===this)closeModal('modal-upcoming')">
            <div class="modal-box">
                <div class="modal-header">
                    ...
                    <button class="modal-close" onclick="closeModal('modal-upcoming')">X</button>
                </div>
                <div class="modal-body">
                    <div class="input-group">
                        <label>Nom</label>
                        <input type="text" id="up-name" placeholder="Ex: JIRAMA">
                    </div>
                    <div class="input-group">
                        <label>Montant (Ar)</label>
                        <input type="number" id="up-amount" placeholder="45000">
                    </div>
                    <div class="input-group">
                        <label>Jour du mois</label>
                        <input type="number" id="up-day" min="1" max="31" placeholder="15">
                    </div>
                    <div class="input-group">
                        <label>Catégorie</label>
                        <select id="up-cat">
                            ${getCategories().filter(c=>c.type==='expense').map(c=>`<option value="${c.id}">${c.name}</option>`).join('')}
                        </select>
                    </div>
                </div>
                <div class="modal-footer">
...
function renderGoals() {
    ...
    document.getElementById('view-goals').innerHTML = `
        <header class="view-header">
            <div>
                <h1 class="view-title">Objectifs d'épargne</h1>
                <p class="view-subtitle">Suivez votre progression</p>
            </div>
            ...
            ${goals.map(g => {
                ...
                return `<div class="goal-card">
                    <div class="goal-card-top">
                        <span class="goal-card-icon">${g.name.charAt(0)}</span>
                        <div style="flex:1">
                            <div class="goal-name" style="font-size:15px">${escapeHtml(g.name)}</div>
                            ${g.deadline ? `<div style="font-size:11px;color:var(--clr-text-3)">Échéance : ${g.deadline}</div>` : ''}
                        </div>
                        <button class="btn-del" onclick="deleteGoal('${g.id}')">X</button>
                    </div>
                    <div class="goal-bar-track" style="margin:12px 0 6px">
                        <div class="goal-bar-fill" style="width:${pct}%;background:${g.color};transition:width .5s"></div>
                    </div>
                    ...
                    <div style="display:flex;gap:8px">
                        <input type="number" class="budget-input" placeholder="  (Ar) " id="add-${g.id}" style="flex:1">
                        <button class="btn-primary" style="padding:6px 14px;font-size:13px" onclick="addToGoal('${g.id}')">+ Épargner</button>
                    </div>
                </div>`;
            }).join('') || '<div class="goal-empty" style="grid-column:1/-1;padding:40px;text-align:center;color:var(--clr-text-3)">Aucun objectif. Créez-en un !</div>'}
        </div>
        ...
        <div id="modal-goal" class="modal-overlay" onclick="if(event.target===this)closeModal('modal-goal')">
            <div class="modal-box">
                <div class="modal-header">
                    ...
                    <button class="modal-close" onclick="closeModal('modal-goal')">X</button>
                </div>
                <div class="modal-body">
                    <div class="input-group">
                        <label>Nom</label>
                        <input type="text" id="g-name" placeholder="Ex: Voyage">
                    </div>
                    <div class="input-group">
                        <label>Objectif (Ar)</label>
                        <input type="number" id="g-target" placeholder="2000000">
                    </div>
                    <div class="input-group">
                        <label>Épargne initiale</label>
                        <input type="number" id="g-saved" placeholder="0">
                    </div>
                    <div class="input-group">
                        <label>Couleur</label>
                        <input type="color" id="g-color" value="#6366f1">
                    </div>
                    <div class="input-group">
...
function saveGoal() {
    ...
    if (!name || !target) { 
        alert('Remplissez au moins le nom et l\'objectif.'); 
        return; 
    }
    ...
    document.getElementById('view-history').innerHTML = `
        <header class="view-header">
            <div>
                ...
                <p class="view-subtitle">Toutes vos transactions (${filtered.length})</p>
            </div>
        ...
        <div class="card" style="margin-bottom:20px">
            <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:center">
                ...
                <select class="filter-select" onchange="histFilter.cat=this.value;renderHistory()">
                    ...
                    ${cats.map(c=>`<option value="${c.id}" ${histFilter.cat===c.id?'selected':''}>${c.name}</option>`).join('')}
                </select>
            ...
            <ul class="tx-list">
                ${filtered.map(tx => {
                    ...
                    return `<li class="tx-item">
                        ...
                        <button class="btn-del" style="margin-left:8px" onclick="deleteTransactionHistory('${tx.id}')">X</button>
                    </li>`;
    ...
    document.getElementById('view-settings').innerHTML = `
        <header class="view-header">
            <div>
                ...
                <p class="view-subtitle">Catégories et préférences</p>
            </div>
        </header>
        ...
        <div class="dash-body">
            <div class="dash-left">
                <div class="card">
                    <div class="card-header">
                        <h2 class="card-title">Catégories de dépenses</h2>
                ...
                <div class="card">
                    <div class="card-header">
                        <h2 class="card-title">Catégories de revenus</h2>
            ...
            <div class="dash-right">
                <div class="card">
                    ...
                    <div style="display:flex;flex-direction:column;gap:14px">
                        <div class="input-group">
                            ...
                            <input type="text" value="Nomena" disabled style="background:var(--clr-bg)">
                        </div>
                        <div class="input-group">
                            <label>Email</label>
                            <input type="email" id="s-email" value="${getData('email','')}">
                        </div>
                ...
                <div class="card">
                    ...
                    <div style="display:flex;flex-direction:column;gap:10px">
                        ...
                        <button class="btn-primary" style="width:100%" onclick="generatePDF()">Rapport mensuel (PDF)</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="modal-cat" class="modal-overlay" onclick="if(event.target===this)closeModal('modal-cat')">
            <div class="modal-box">
                <div class="modal-header">
                    ...
                    <button class="modal-close" onclick="closeModal('modal-cat')">X</button>
                </div>
                <div class="modal-body">
                    <input type="hidden" id="cat-type">
                    <div class="input-group">
                        <label>Nom</label>
                        <input type="text" id="cat-name" placeholder="Ex: Abonnements">
                    </div>
                    <div class="input-group">
                        <label>Couleur</label>
        ...
        <div style="font-family: Arial, sans-serif; padding: 40px; color: #1a1917;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
                ...
                <tbody>
                    ...
                    }).join('') || '<tr><td colspan="4" style="padding: 20px; text-align: center; color: #a8a29e;">Aucune transaction ce mois</td></tr>'}
                </tbody>
            </table>
            ...
            }).join('') || '<p style="color: #a8a29e;">Aucun objectif</p>'}
